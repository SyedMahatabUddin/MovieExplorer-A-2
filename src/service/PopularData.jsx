    ...
    <div className="w-full flex flex-col items-center">
     ...
     <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 w-full">
         ...
         <div key={user.id}  className="max-w-300">
          ...
          <p className="font-semibold">{user.show?.name || user.name}</p>

          <div className="flex justify-between"> 
            <p className="flex items-center gap-1 text-[14px]"><Star size={14} strokeWidth={2} className="text-yellow-400 fill-yellow-400" />{user.rating?.average ||"N/A"} </p>
        ...
        {modal=== user.id && (<div onClick={()=>setModal(true)} className="fixed inset-0 bg-black/50 flex items-center justify-center  px-4 z-50">
          ...
          <div className="grid grid-cols-2">
           ...
           <h2 className="text-xl font-extrabold text-center my-2">{user.name}</h2> 
          <p><span className="font-semibold text-gray-800">Type:</span> {user.type}</p>
          <p><span className="font-semibold text-gray-800">Language:</span> {user.language}</p>
          <p><span className="font-semibold text-gray-800">Runtime:</span> {user.runtime} min</p>
          <p><span className="font-semibold text-gray-800">Rating:</span> {user.rating?.average|| "N/A"}</p>
           </div>


          </div>
          <p className="text-sm text-gray-700 w-full text-center px-3">
          {user.summary?.replace(/<[^>]+>/g, "")|| 'No summary available for this show yet.'}
        </p>
